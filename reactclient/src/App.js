...
export default function App() {
  ...
  function getPosts() {
    const url = Constants.API_URL_GET_ALL_POSTS; //"https://localhost:7235/get-all-posts";
    fetch(url, {
      method: "GET",
    })
  ...
  function onPostUpdated(updatedPost) {
    ...
    alert(
      `Post successfully updated. After clicking OK, look for the post with the title "${updatedPost.title}" in the table below to see the updated post.`
    );
  }

  function opPostDeleted(dletedPostPostId) {
    let postCopy = [...posts];

    const index = postCopy.findIndex((postcopyPost, currentIndex) => {
      if (postcopyPost.postId === dletedPostPostId) {
        return true;
      }
    });
